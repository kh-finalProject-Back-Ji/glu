...
if (updateInfo != null) {
	...
	updateInfo.addEventListener("submit", async (e) => {
		...
		if (currentNickname !== memberNickname.value) {
			...
			//          -> "이 작업이 끝날때까지 기다려주세요."
			const resp = await fetch("/8gyosi/checkNickname?memberNickname=" + memberNickname.value);
...
let previousImage = (profileImg !== null) ? profileImg.src : null;
